...
SYN_DAYS, PARTIAL_VOL_DAYS, NO_VOL_DAYS = set(), set(), set()
for _k in range(1, _N):
    _same = _tot = _zero = 0; _ratios = []
    for _s in _cur:
    ...
    # a day re-derived from the next day's net change carries exact closes and no
    # volume at all (extend_series_gap.py): its price is real, its volume absent
    if _tot and _zero / _tot > 0.90: NO_VOL_DAYS.add(_k)
    if _ratios and statistics.median(_ratios) < 0.7: PARTIAL_VOL_DAYS.add(_k)
SYN_DAYS -= NO_VOL_DAYS
BAD_VOL_DAYS = SYN_DAYS | PARTIAL_VOL_DAYS | NO_VOL_DAYS
print("copied days:", [CAL[k] for k in sorted(SYN_DAYS)],
      "| partial-volume days:", [CAL[k] for k in sorted(PARTIAL_VOL_DAYS)],
      "| price-only days (no volume):", [CAL[k] for k in sorted(NO_VOL_DAYS)])
...
for sym, (fi, cs, vs, ff) in SER.items():
    ...
    if not is_common_stock(sym, _m.get("name", sym), _m.get("industry", "")):
        ...
        continue
    # liquidity over the last 20 days that carry a volume figure: a copied,
    # partial or price-only day would drag the median for reasons of our own data
    _gv = [j for j in range(len(cs)) if (fi + j) not in BAD_VOL_DAYS][-20:]
    dv = [cs[j] * vs[j] for j in _gv]
    if not dv or statistics.median(dv) < 1_000_000: continue
    stats_counts["liq"] += 1
...
out = {"meta": {"last_date": LAST_DATE, "cal_first": CAL[0], "cal_last": CAL[-1],
                ...
                "price_only_days": [CAL[k] for k in sorted(NO_VOL_DAYS)],
