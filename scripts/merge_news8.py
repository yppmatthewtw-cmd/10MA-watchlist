#!/usr/bin/env python3
"""Merge R8 news inputs into data/news8.json — one entry per listed ticker.

R8 carries the R7 research forward for every ticker it already covered and adds
the names new to the R8 lists (same 2026-09-01 close; the review's data fixes changed the membership), so only genuinely new tickers need fresh
research.

Sources:
  data/news7.json           research carried over from R6
  scratchpad r8_res_*.json  condensed text + hot + catalyst for the new names
  scratchpad r8_redo_*.json re-runs for a new name a first pass could not cover
...
prev = json.load(open(f"{SCRATCH}/news8.json"))
...
res_files = numbered(f"{AGENT_DIR}/r8_res_*.json") + numbered(f"{AGENT_DIR}/r8_redo_*.json")
redone = []
for f in res_files:
