...
REV = os.environ.get("REV", "R10.00")
_rv = f"{SCRATCH}/{os.environ.get('REVIEW_JSON', 'review10.json')}"
...
def c7_cells(r, pr):
    def parts(row):
        ...
        return {
            ...
            "retr": retr_s,
            "held": f'{c["d_held"]}' + ('<span class="cwarn" title="期間曾跌穿底部，該項打 0.25 折">⚠</span>' if c["undercut"] else ""),
            "dvr": (f'{c["dv_ratio"]:.2f}', c["dv_ratio"] < 0.85),
            "contr": (f'{c["contr"]:.2f}', c["contr"] < 0.6),
            "rs": (f'{c["rs21_pct"]:+.0f}' if abs(c["rs21_pct"]) >= 100 else f'{c["rs21_pct"]:+.1f}',
                    c["rs21_pct"] > 0),
            "maf": (f'{maf}/3', maf == 3),
...
CATL_TH = ('<th class="srt" data-k="catl" title="一句講清楚隻股喺咩事件下先至由底回升：日期 · 事件 · 效果">'
           '催化<span class="thn">點解由底回升</span></th>')

# The seven evidence items hold a tick or a number, so their headings are one
# word each and the definition lives in the tooltip and in the legend below the
# table. Before R10 each carried a two-line heading and the column inherited that
# width, which is where most of the table's empty space came from.
C7_HEADS = (
    '<th class="srt c7h" data-k="brk" title="突破：最近兩個底之間嘅中間高位有冇被升穿（佔確定性 25%）">突破</th>'
    '<th class="srt c7h" data-k="retr" title="回補：收復最後一段跌幅嘅百分比（10%）">回補</th>'
    '<th class="srt c7h" data-k="held" title="守底：最後一個底已守嘅交易日數，15 日滿分；曾跌穿打 0.25 折（15%）">守底<span class="thn">日</span></th>'
    '<th class="srt c7h" data-k="dvr" title="量比：近 15 日跌日對升日嘅成交量比，越低越好（15%）">量比</th>'
    '<th class="srt c7h" data-k="contr" title="遞減：末段跌幅 ÷ 首段跌幅，越低代表收縮越明顯（10%）">遞減</th>'
    '<th class="srt c7h" data-k="rs" title="RS：21 日回報減全體中位數（10%）">RS<span class="thn">%</span></th>'
    '<th class="srt c7h" data-k="maf" title="均線：價 &gt; 20MA、20MA &gt; 50MA、50MA 向上，三項計分（15%）">均線</th>')

# Column widths live here rather than in the content: the table is laid out fixed
# so a page fits a 1400px window without sideways scrolling. Order matches <th>.
W_P1 = [30, 124, 84, 140, 66, 60, 60, 84, 44, 38, 34, 38, 36, 38, 34, 92, 150, 186]
W_SUB = [30, 124, 84, 140, 60, 60, 44, 38, 34, 38, 36, 38, 34, 92, 194, 234]


def colgroup(widths):
    """Fixed widths for every column except the last two (the two reason texts).

    Those two are left unsized on purpose: with table-layout:fixed they absorb
    whatever the window has spare, so a wide screen gives the paragraphs room
    (and shorter rows) instead of leaving a strip of empty space on the right,
    while a narrow one falls back to the table's min-width and scrolls."""
    cols = "".join(f'<col style="width:{w}px">' for w in widths[:-2]) + "<col><col>"
...
def tick_cell(r, pr, L=None):
    ...
    warn = (f' <span class="warn{" chg" if warn_now != warn_prev else ""}" title="收市低於本頁時間框嘅 MA">⚠</span>' if warn_now
            else (' <span class="chg" title="上一版收市低於 MA，本版已重上">↑MA</span>' if warn_prev else ""))
    ...
    ma_d = ""   # the MA's own move is in the slope column; keep this cell to one line
    sub = r["gsub"] if (r["sp500"] and r.get("gsub")) else r["industry"]
    sec_chg = bool(pr) and (pr["sector_zh"], pr["industry"]) != (r["sector_zh"], r["industry"])
    sect = (f'<span class="sectag{" chg" if sec_chg else ""}" '
            f'title="{esc(r["sector"])} · {esc(sub)}">{esc(r["sector_zh"])}</span>')
    return (f'<div class="tk"><a href="{tv_url(r)}" target="_blank" rel="noopener">{esc(r["sym"])}</a>'
            f'<span class="ex">{esc(r["exch"])}</span>{sp}{newb}'
            f'<em title="{esc(nm)}">{esc(nm if len(nm) <= 22 else nm[:21] + "…")}</em>'
            f'<span class="pxl nums" title="{esc(ml)} {r["ma"]:g}">{r["close"]:g}{px_d}'
            f' <span class="mut">/{r["ma"]:g}</span>{ma_d}{warn}</span>'
            f'{cap}{sect}</div>')

def spark_cell(r, pr):
    sl_chg = same_basis(r, pr) and pr["slope"] != r["slope"]
    # the bottom sequence had its own column until R10; it now rides along here,
    # where the green dots already mark the same bottoms
    chain = " → ".join(f"{d[5:]} ${p:g}" for d, p in r["hl"])
    return (f'<div title="底部序列（45 日內逐個遞升）：{esc(chain)}">{spark_svg(r["spark"])}'
            f'<div class="subsc nums slope{" chg" if sl_chg else ""}" '
            f'title="MA{r["L"]} 較 {r["W"]} 個交易日前">{r["slope"]:+.2f}%</div></div>')
...
def table_sub(pid):
    pg = O["pages"][pid]
    L = pg["L"]
    head = (f'<tr><th title="綜合分數排名">#</th><th>Ticker · 現價/MA{L} · 市值</th>'
            ...
            f'{C7_HEADS}'
            f'<th class="srt" data-k="slope" title="60 日收盤同 MA 走勢，綠點 = 底；底部序列喺格內 tooltip">走勢<span class="thn">斜率</span></th>'
            f'<th>下跌原因</th>'
            f'<th>回升原因<span class="thn">熱炒 highlight · 信心</span></th></tr>')
    rows = []; n_new = n_chg = 0
    for i, r in enumerate(pg["rows"], 1):
        ...
        rows.append(
            ...
            f'<td class="whyc">{why_recovery(r["sym"])}</td></tr>')
    return head, "".join(rows), pg, n_new, n_chg

def table_page1():
    head = (f'<tr><th title="爆發潛力排名">#</th><th>Ticker · 現價/MA · 市值</th>'
            f'<th class="srt" data-k="cat">主要催化劑<span class="thn">熱炒 news-driven</span></th>'
            f'{CATL_TH}'
            ...
            f'{C7_HEADS}'
            '<th class="srt" data-k="slope" title="60 日收盤同 MA10 走勢，綠點 = 底；底部序列喺格內 tooltip">走勢<span class="thn">MA10 斜率</span></th>'
            '<th>下跌原因</th>'
            '<th>回升原因<span class="thn">熱炒 highlight · 信心</span></th></tr>')
...
.wrap{max-width:2200px;margin:0 auto;padding:26px 16px 60px}
h1{font-size:19px;margin:0 0 3px}
.h1s{font-weight:400;font-size:12.5px;color:var(--ink2);margin-left:10px}
.docs{margin-top:26px;border-top:1px solid var(--grid);padding-top:16px}
.dochead{font-size:12px;color:var(--mut);margin:0 0 10px;font-weight:600}
.below{margin-top:10px}
.sub{color:var(--ink2);font-size:11.5px;margin-bottom:8px}
...
/* Widths come from the colgroup, not from the content, so a page fits a 1400px
   window; the wrapper still scrolls on anything narrower. */
table{border-collapse:collapse;width:100%;table-layout:fixed;font-size:11.5px}
th{position:sticky;top:var(--navh,0px);text-align:left;font-size:10.5px;color:var(--mut);font-weight:600;
 padding:6px 5px;border-bottom:1px solid var(--grid);background:var(--sf);white-space:nowrap;z-index:1;
 -webkit-user-select:none;user-select:none}
th.c7h{text-align:right;padding-left:2px;padding-right:5px}
...
.thn{display:block;font-weight:400;font-size:9px}
td{padding:4px 5px;border-bottom:1px solid var(--grid);vertical-align:middle;overflow:hidden}
td.c7{text-align:right;padding-left:2px}
...
.tk em{display:block;font-style:normal;font-size:9.5px;color:var(--mut);max-width:116px;
 overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
.sectag{display:inline-block;margin-top:2px;margin-left:3px;font-size:9px;color:var(--ink2);
 border:1px solid var(--ring);border-radius:7px;padding:0 4px;white-space:nowrap}
.tk .pxl{display:block;font-size:10.5px;margin-top:2px;white-space:normal}
.capb{display:inline-block;margin-top:2px;font-size:9px;color:var(--ink2);background:var(--capbg);
 border:1px solid var(--ring);border-radius:7px;padding:0 4px;font-variant-numeric:tabular-nums;white-space:nowrap}
...
.catc{max-width:none}
.catlc{max-width:none}
.catl{font-size:10.5px;line-height:1.45;color:var(--ink2);display:-webkit-box;-webkit-line-clamp:4;
...
.catb{display:inline-block;background:var(--hotbg);color:var(--hotink);border:1px solid var(--hotbd);
 border-radius:6px;padding:2px 5px;font-size:11px;font-weight:700;line-height:1.3}
...
.vcpb{display:block;min-width:0}
.vcpb .meter{height:4px;border-radius:2px;background:var(--meter);width:100%;margin-bottom:2px}
.vcpb .meter i{display:block;height:100%;border-radius:3px;background:var(--seq)}
.vcpb b{font-variant-numeric:tabular-nums;font-size:12px}
...
.c7{font-size:11px;white-space:nowrap;font-variant-numeric:tabular-nums}
...
.whyc{padding-right:6px}
/* three lines by default, full text on the 展開全文 toggle (and in the tooltip) */
.why{font-size:10.5px;line-height:1.45;color:var(--ink2);display:-webkit-box;-webkit-line-clamp:3;
 -webkit-box-orient:vertical;overflow:hidden}
body.expand .why{display:block;-webkit-line-clamp:unset}
body.expand .catl{-webkit-line-clamp:unset}
...
.fr{display:inline-block;font-size:9px;color:var(--mut);border:1px dashed var(--axis);
 border-radius:7px;padding:0 3px;margin:1px 2px 1px 0;line-height:1.5}
.fr.on i{margin-left:1px}
...
<div class="tblwrap"><table style="min-width:{sum(W_P1)}px;width:100%">{colgroup(W_P1)}<thead>{head1}</thead><tbody>{body1}</tbody></table></div>
<div class="below">{pghead1}{drop1}{cat_chips(O["page1"])}{sector_chips(O["page1"])}{LEGEND}</div>
</section>''')
...
<div class="tblwrap"><table style="min-width:{sum(W_SUB)}px;width:100%">{colgroup(W_SUB)}<thead>{head}</thead><tbody>{body}</tbody></table></div>
<div class="below">{pghead}{drop}{cat_chips(pg["rows"])}{sector_chips(pg["rows"])}{LEGEND}</div>
</section>''')
...
sort_btns = ('<span class="sep"></span><span class="slab">排序：</span>'
             ...
             + '<button class="cbtn" id="expand" title="下跌／回升原因預設只顯示三行，撳一下顯示全文">展開全文</button>'
...
(function() {
  ...
  if (expandBtn) {
    var expanded = false;
    try { expanded = localStorage.getItem('ma10expand') === '1'; } catch (e) {}
    var paintExpand = function() {
      document.body.classList.toggle('expand', expanded);
      expandBtn.classList.toggle('on', expanded);
      expandBtn.textContent = expanded ? '收起全文' : '展開全文';
    };
    expandBtn.addEventListener('click', function() {
      expanded = !expanded;
      try { localStorage.setItem('ma10expand', expanded ? '1' : '0'); } catch (e) {}
      paintExpand();
    });
    paintExpand();
  }
...
<div class="wrap">
<h1>10MA Uptrend Watchlist {RNAME}<span class="h1s">一底高於一底 × VCP × 底部確定性 · 大中小型股分頁</span></h1>
<div class="sub">數據至 <b>{M["last_date"]}</b> 收盤 · 全美掃描 {c["liq"]:,} 隻合資格 · 13 頁：總表＋4 個時間框 × 大／中／小型股 · 13 頁一次過睇曬（<b>唔使向右捲</b>） · 說明全部喺表下面 · 頂欄可切換<b>淺色／深色</b>、<b>展開全文</b> · 相對 {esc(PREV_REV)} 嘅更新以灰色小字標示 · 欄標題可排序、欄寬可拖曳</div>
<nav class="nav">
...
<h2 class="dochead">以下為說明：本版更新 · 市場背景 · 篩選規則 · 數據來源</h2>
{upd_html}
{mkt_html}
{rules_html}
{foot}
