#!/usr/bin/env python3
"""Merge R6 news inputs into data/news6.json — one entry per listed ticker.

R6 carries the R5 research forward for every ticker it already covered and adds
the names new to the 2026-08-31 scan, so only genuinely new tickers need fresh
research.

Sources:
  data/news5.json           research carried over from R5 (178 tickers)
  scratchpad r6_res_*.json  condensed text + hot + catalyst for the new names
  scratchpad r6_redo_*.json re-runs for a new name a first pass could not cover
...
scr = json.load(open(f"{SCRATCH}/screen_results6.json"))
need = [r["sym"] for r in scr["page1"]]
prev = json.load(open(f"{SCRATCH}/news5.json"))
...
res_files = numbered(f"{AGENT_DIR}/r6_res_*.json") + numbered(f"{AGENT_DIR}/r6_redo_*.json")
redone = []
for f in res_files:
...
json.dump(out, open(f"{SCRATCH}/news6.json", "w"), ensure_ascii=False, indent=1)
print("wrote", f"{SCRATCH}/news6.json")
