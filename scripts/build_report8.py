...
N = json.load(open(f"{SCRATCH}/{os.environ.get('NEWS_JSON', 'news8.json')}"))
...
REV = os.environ.get("REV", "R8.00")
_rv = f"{SCRATCH}/{os.environ.get('REVIEW_JSON', 'review8.json')}"
...
MODEL_TAG = os.environ.get("MODEL_TAG", "claudefable51xhigh")
...
css = """
/* Dark-only. Palette lives on bare :root; every text token clears WCAG AA on --sf and the --hl row hover.
   --chg is the "updated in this revision" red: 6.3:1 on --sf, 5.9:1 on --hl. */
:root{color-scheme:dark;
 ...
 --chg:#ff6b6b;--chgbg:rgba(255,107,107,.12);--chgbd:#c94848}
...
.mkt .mf span.chg{border-color:var(--chgbd);background:var(--chgbg);color:var(--chg)}
.mkt .mf span.chg b{color:var(--chg)}
/* the revision-delta card: red-bordered, everything inside is what changed */
.upd{border-color:var(--chgbd);background:var(--chgbg)}
.upd h2{color:var(--chg)}
...
.upd .k{color:var(--chg);font-weight:700}
.chg{color:var(--chg)!important}
.chg b,.chg i,.chg em{color:var(--chg)!important}
.dlt{font-size:10px;font-weight:700;margin-left:3px;white-space:nowrap}
td.chg{background:var(--chgbg)}
.why.chg{border-left:2px solid var(--chgbd);padding-left:6px}
.botwrap.chg{border-left:2px solid var(--chgbd);padding-left:6px}
.sect.chg{border-left:2px solid var(--chgbd);padding-left:6px}
.catb.chgb{border-color:var(--chg);box-shadow:0 0 0 1px var(--chg) inset}
tr.rownew td{background:var(--chgbg)}
tr.rownew td:first-child{border-left:3px solid var(--chg)}
.newb{display:inline-block;font-size:9px;font-weight:700;color:var(--pg);background:var(--chg);border-radius:6px;
 padding:1px 5px;margin-left:6px;vertical-align:1px}
.rflag{display:inline-block;margin-top:4px;font-size:9.5px;font-weight:700;color:var(--chg);border:1px dashed var(--chg);
 border-radius:7px;padding:1px 6px;white-space:nowrap}
.dropped{font-size:11.5px;color:var(--ink2);border:1px dashed var(--chgbd);border-radius:8px;padding:6px 10px;margin-bottom:10px;
 line-height:1.9}
.dropped .dlab{color:var(--chg);font-weight:700;margin-right:4px}
.dropped b{color:var(--chg);margin-left:8px}
...
.nav button.cbtn{border:1px solid var(--chgbd);color:var(--chg);padding:7px 12px;font-size:12px}
.nav button.cbtn.on{background:var(--chg);color:var(--pg);border-color:var(--chg)}
...
.chip.red{border-color:var(--chgbd);color:var(--chg)}
.chip.red i{color:var(--chg)}
...
<h2>本版更新 · {esc(REV)} 對比 {esc(PREV_REV)}（頁內所有紅色 = 本版更新內容）</h2>
...
<div class="sub">數據至 <b>{M["last_date"]}</b> 收盤 · 全美掃描 {c["liq"]:,} 隻合資格 · 13 頁：總表＋4 個時間框 × 大／中／小型股 · <span class="chg">紅色 = 相對 {esc(PREV_REV)} 更新嘅內容</span> · VCP／確定性／7項證據可排序 · 欄寬可拖曳</div>
{upd_html}
{rules_html}
<nav class="nav">
