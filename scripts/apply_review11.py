...
# why did the leavers leave: last bottom undercut on close, or just the MA test
broke, ma_only, gone = [], [], []
for s in out_syms:
    if s not in SER or SER[s][0] + len(SER[s][1]) != N:
        gone.append(s); continue
    pl = plisted[s]["hl"][-1][1] if plisted[s].get("hl") else None
    c = SER[s][1][-1]
    (broke if pl and c < pl else ma_only).append(s)
...
notes = [
    {"title": f"[本版數據] 更新至 {last} 收盤",
     "text": f"新增 09-04（周五）一個交易日（合共 {scr['meta']['n_days']} 日；09-04 快照由本 repo 嘅 GitHub Actions 抓 Nasdaq screener，"
             ...
             f"跌出嘅 {n_out} 隻：{len(broke)} 隻收市已經跌穿最後一個底（{J(broke)}），"
             f"{len(ma_only)} 隻係四個時間框嘅 MA 條件全部唔再成立"
    ...
    {"title": "[本版觀察] 09-04 非農強 → 加息機率回升，名單點反應",
     "text": f"09-04 全體 $10 億市值以上股份中位數 {u_med:+.2f}%、{u_up:.1f}% 上升；總表 {len(listed)} 隻中位數 {p1_med:+.2f}%，"
             ...
             f"所以仍然喺榜，但「守底」已經名存實亡。{len(p1_peak_today)} 隻嘅底部後最高位就係 09-04 當日。",
     "tickers": p1_down[:10]},
    {"title": "[已修正] 催化欄逐行對照序列（本版重新計算）",
     "text": f"{dated} 句有日期、{undated} 句冇日期（原文本身冇提日期，唔憑空補）、{no_cat} 句係「無個股催化」。"
             ...
             f"呢啲事件係造成低位嘅原因多過回升嘅原因，全部自動加「事件日 −X%」標記；另有 {len(flat_days)} 句所指嗰日波幅喺 ±1% 之內"
             "（多數係併購釘價股）。新上榜嘅催化句由今次研究直接寫，沿用嘅由 R9 帶過嚟，全部再經 news_checks.py 對照序列。",
    ...
    {"title": "[已加標記] 靠 09-02 先成立嘅結構",
     "text": f"總表 {len(dep_bottom)} 行嘅最後一個底係靠 09-02 呢個反推出嚟嘅 bar 先夠三日確認；另外如果索性剝走 09-02，"
...
for n in open_notes:
    ...
    if t.startswith("[待你決定] 確定性飽和"):
        ...
        if not pairs:
            n["text"] = "本版總表已經冇同一公司嘅雙類股同時上榜（R9 嘅 NWS／NWSA 已跌出），但規則本身未改，將來仍會出現。" + n["text"].split("建議", 1)[-1].join(["建議", ""]) if "建議" in n["text"] else n["text"]
            n["tickers"] = []
...
for i, f in enumerate(agent_notes[:6], 1):
    ...
    notes.append({"title": f"{tag} {f.get('title', '')[:60]}",
                  "text": (f.get("evidence", "")[:400] + " → " + f.get("proposed_fix", "")[:200]).strip()})
...
headline = (
    ...
    f"（{len(broke)} 隻跌穿最後一個底、{len(ma_only)} 隻 MA 條件唔再成立）。名單本身當日中位數 {p1_med:+.2f}%，{len(p1_down)} 隻跌超過 1%，"
    f"{len(p1_undercut)} 隻收市已低過最後一個底但未夠三日確認。新上榜以 {new_sectors.split('、')[0]}、{new_sectors.split('、')[1]} 為主"
    f"（{new_caps}），催化劑 {new_kinds.split('、')[0] if new_kinds else '無'} 佔多。"
...
print(f"wrote {SCRATCH}/{OUT} · flags {len(flags)} (dropped {len(dropped_flags)}: {dropped_flags}) · "
      f"catalyst warnings {len(warns)} · notes {len(notes)} · new {n_new} · out {n_out} (broke {len(broke)}, ma {len(ma_only)}, gone {len(gone)})")
