#!/usr/bin/env python3
"""Build the 10MA R5 report: 1 summary page + 12 timeframe x market-cap sub-pages.
...
N = json.load(open(f"{SCRATCH}/news5.json"))
...
OUTNAME = f"10MA_uptrend_watchlistGit_R5.00_claudeopus5xhigh_{STAMP}.html"
...
:root[data-theme="dark"]{color-scheme:dark;--pg:#0d0d0d;--sf:#1a1a19;--ink:#fff;--ink2:#c3c2b7;
 ...
 --okbg:#15230f;--nobg:#2a1c1c;--hotbg:#4a3512;--hotink:#ffd479;--hotbd:#a8791f;--capbg:#232322}
...
<h2>篩選規則（10MA R5 · 每個時間框再分大／中／小型股，共 12 個子頁）</h2>
...
② 數據重建：GitHub 每日 Nasdaq 快照鏡像（zyhe16/top-us-stock-tickers）逐 commit 重建每日收盤序列，共 {M["n_days"]} 個交易日（{M["cal_first"]} → {M["cal_last"]}）；4 日無快照以前值填補；08-27 收盤以官方 net-change 校正。重建序列與 20MA R3 報告交叉核對：74/74 上榜股現價完全一致。<br>
...
(function() {
  ...
  try {
    var sv = JSON.parse(localStorage.getItem('ma10r5') || 'null');
...
html_doc = f"""<title>10MA Uptrend Watchlist R5</title>
<style>{css}</style>
<div class="wrap">
<h1>10MA Uptrend Watchlist R5（一底高於一底 × VCP × 底部確定性 · 大中小型股分頁）</h1>
